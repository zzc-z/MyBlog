...
class card extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showIndex: 0,
      refs: [this.about, this.experience, this.contact],
    };
  }
  handleButtonClick(index) {
    let { showIndex, refs } = this.state;
    showIndex = index;
    this.setState({
      showIndex,
    });
    let keys = refs[index].current.className.substring(0);
    // if (showIndex == refs[index].keys) {
    //   refs[index].className == "is-active"
    // }else{
    //   refs[index].className == ""
    // }
    console.log(keys);
  }
  render() {
      ...
      <div className="card" data-state="#about">
        ...
        <div className="card-main">
          ...
          <div className="card-buttons">
            {this.state.cardButtom.map((item, index) => {
              return (
                <button
                  onClick={() => this.handleButtonClick(index)}
                  data-section={item.dataSection}
                  key={index}
                  className={index === this.state.showIndex ? "is-active" : ""}
                >
                  {item.list}
                </button>
              );
